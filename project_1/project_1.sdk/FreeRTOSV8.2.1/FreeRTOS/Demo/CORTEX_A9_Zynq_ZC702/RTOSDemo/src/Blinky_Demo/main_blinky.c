...
static void prvBlinkFrequencyTask(void *pvParameters);

/*-----------------------------------------------------------*/

/* The queue used by both tasks. */
static QueueHandle_t xQueue = NULL;
static int blinkFrequency = 2;
...
void main_blinky(void) {
	...
	if (xQueue != NULL) {
		...
		xTaskCreate(prvBlinkFrequencyTask, "blinkFreq", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 3, NULL);
...
static void prvQueueSendTask(void *pvParameters) {
	...
	for (;;) {
		...
		vTaskDelayUntil(&xNextWakeTime, 1000 / blinkFrequency / 2);
...
static void prvBlinkFrequencyTask(void *pvParameters) {
	...
	xNextWakeTime = xTaskGetTickCount();
	for (;;) {
		blinkFrequency = blinkFrequency + 2 > 10 ? 2 : blinkFrequency + 2;
